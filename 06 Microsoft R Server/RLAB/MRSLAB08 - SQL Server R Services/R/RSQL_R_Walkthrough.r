# PreRequisites: You have installed Revolution R Enterprise 8.0 or higher on the machine and SQL Server 2016 or higher on the database server
...
connStr <- "Driver=SQL Server;Server=.;Database=nyctaxi;Trusted_connection=Yes"
...
sampleDataQuery <- "select top 10000 tipped, fare_amount, passenger_count,trip_time_in_secs,trip_distance, 
...
# Need to switch back to local context as rxExec is not supported on SQL Server
...
rxSetComputeContext(cc)
rxRocCurve( actualVarName = "tipped", predVarNames = "Score", scoredOutput)
